...
std::vector<QueryMetadata> GenerateQueries(
    const TestSettingsInternal& settings,
    const LoadableSampleSet& loaded_sample_set, SequenceGen* sequence_gen,
    ResponseDelegate* response_delegate,int dnnIndex) {
  auto tracer =
      MakeScopedTracer([](AsyncTrace& trace) { trace("GenerateQueries"); });
  ...
  while (prev_timestamp < gen_duration || queries.size() < min_queries) {
    ...
    queries.emplace_back(samples, timestamp, response_delegate, sequence_gen,dnnIndex);
    prev_timestamp = timestamp;
    timestamp += schedule_distribution(schedule_rng);
  }
...
PerformanceResult IssueQueries(SystemUnderTest* sut,
  ...
  #if MULTI
    //체크:n 개수만큼 for. GenerateQueries  
    int sutN=2;
    std::vector<std::vector<QueryMetadata>> queries_2d; 

    for(int i=0;i<sutN;i++)
    {
      queries_2d.push_back(GenerateQueries<scenario, mode>(
        settings, loaded_sample_set, sequence_gen, &response_logger,i));  //다른 벡터에 복사하지 않고 바로 삽입하는 방식으로 사용함.
    }
    std::vector<QueryMetadata> queries = GenerateQueries<scenario, mode>(
        settings, loaded_sample_set, sequence_gen, &response_logger,0); //^체크 : 이 함수 하단에서 마지막 쿼리까지 보내졌는지 체크하는 부분이 있음. 쿼리 하나가지고 수행하고있음
  #else
    std::vector<QueryMetadata> queries = GenerateQueries<scenario, mode>(
  ...
  // Create and initialize an IssueQueryState.
    ...
    IssueQueryState state{
        sut, &queries_2d, &response_logger, &settings, mode, sutN, {}, {}, false, 0,
        0,   {}};  
  #else
    IssueQueryState state{
  ...
  #if MULTI
    controller.StartIssueQueries(&state,&multi,scenario);
  #else
  ...
  // is done with them.
  auto& final_query = queries[queries_issued - 1];  //^체크: 쿼리의 마지막을 체크하는 부분
...
void RunAccuracyMode(SystemUnderTest* sut, QuerySampleLibrary* qsl,
  ...
  for (auto& loadable_set : loadable_sets) {
    ...
    //IssueQueries 하나에서 모두 수행됨
        //IssueQueries를 실행하고 그 내용으로 pr생성, 이 코드가 종료됨은 모든 쿼리의 수행이 끝남을 의미함
    {
      auto tracer = MakeScopedTracer(
          [count = loadable_set.set.size()](AsyncTrace& trace) {
...
void startMultiTest(SystemUnderTest* sut, QuerySampleLibrary* qsl,
                    const TestSettings& requested_settings,
                    const LogSettings& log_settings) {
  ...
  std::cout<<"in loadgen startMultiTest\n";
...
void QueryReceiveComplete(ResponseId id)
{
  PerfClock::time_point timestamp = PerfClock::now();
  loadgen::SampleMetadata* sample = reinterpret_cast<loadgen::SampleMetadata*>(id);
  loadgen::QueryMetadata* query = sample->query_metadata;
  query->NotifyOneSampleCompleted(timestamp);
}

void QuerySamplesResponseLogging(QuerySampleResponse* responses, size_t response_count,
...
void QuerySamplesComplete(QuerySampleResponse* responses, size_t response_count,
  ...
  //// Notify first to unblock loadgen production ASAP.-지우면 쿼리 전송
  ...
  //multi.isSend=true;
  //multi.sigDnn=1; //sut에게 받은 dnn 번호
}
