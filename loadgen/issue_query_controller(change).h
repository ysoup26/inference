...
class QueryMetadata {
 ...
 public:
  std::vector<QuerySample> query_to_send;   //샘플에 대한 정보
...
struct IssueQueryState {
  ...
  SystemUnderTest* sut;
  //std::vector<QueryMetadata>* queries; //여러 queries를 가리키는 포인터배열로 변경
  std::vector<std::vector<QueryMetadata>> *queries_2d; //쿼리를 저장하는 벡터의 포인터
...
class IssueQueryController {
 public:
  ...
  void StartIssueQueries(IssueQueryState* s,MultiQuery* m,TestScenario scenario);

  /// \brief Notify the IssueQueryThreads to end.
  void EndThreads();
  static void* query_send_thread(IssueQueryController *controller);//param &prm);
 ...
 private:
  ...
  void IssueQueriesInternal(size_t query_stride, size_t thread_idx,TestScenario scenario, bool multi_thread);

  //template <TestScenario scenario, bool multi_thread>
  
  template <typename t>
  void myThreadF(t i){std::cout<<"myThread"<<i<<std::endl;};

  // template <TestScenario scenario, bool multi_thread>
  // static void* threadHelper(void* obj, void* arg) {
  //       auto args = *static_cast<std::pair<std::function<void()>, std::pair<std::pair<scenario, multi_thread>, param>>*>(arg);
  //       args.first();
  //       // auto args = *static_cast<std::pair<scenario,multi_thread>*>(arg)
  //       // //param* i = static_cast<scenario*,multi_thread*>(arg);
  //       // static_cast<IssueQueryController*>(obj)->myThreadF(args);
  //       return nullptr;
  // }
  ...
  bool end_test{false};
  MultiQuery* multiQ; //멀티DNN을 위한 쿼리정보
  param *prm; //lg스레드의 매개변수

  //template<typename a=int>
  //void NN(a i){std::cout<<"[<]"<<i<<"run!\n"<<std::endl;};
};
